...
mfPkg.langUpdate = function(lang, strings, meta, lastSync) {
    ...
    //caching the language on server
    keys = {}
    tmp = mfPkg.mfStrings.find({lang: lang, removed: {$ne: true}}, {fields:{key:1}}).fetch()
    _.each(tmp, function(item){keys[item.key] = 1});
	/*
    ...
    console.log('starting language extratction update.');
    console.log('please remove server/mfExtracts.js after you run this once. and NEVER! check it in.')
	for (key in strings) {
		str = strings[key];
		//existing = this.strings[lang][key];
        existing = mfPkg.mfStrings.findOne({lang: lang, key:key})
        //console.log(_.size(this.strings[lang]));
        mfPkg.extracted.push(key)
        if (existing) {
            stats.existing++;
            delete keys[key];
        }

        if (existing & false) { // removes duplicates if there are one. remove the false to enable it temporary.
            dups = mfPkg.mfStrings.find({lang: lang, key:key}).fetch()
            if (dups.length > 1) {
                //stats.duplicates[key] = dups.length;
                _.each(dups, function(dup) {
                    if (dup._id !== existing._id){
                      stats.removing++;
                      mfPkg.mfStrings.remove({_id:dup._id});
                    }
                });
                console.log('removing '+ (dups.length-1) +  'dups from' + key);
            }
        }
    ...
    _.each(keys, function(index, key) {
      mfPkg.mfStrings.update({key:key, lang:lang}, {$set:{removed:true}}); //removing keys that are no longer present.
    });

    console.log(mfPkg.extracted.length + ' translation keys extracted. stats: ');
    console.log(stats);
    console.log('keys that are too much: ' + _.size(keys));
}
...
//subscription for the translation UI
